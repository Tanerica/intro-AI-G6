...
class agent: # tác tử 
    ...
    def __init__(self,parentMaze,x=None,y=None,shape='square',goal=None,filled=False,footprints=False,color:COLOR=COLOR.blue):
        ...
        if(isinstance(color,str)):
            if(color in COLOR.__members__):
                self.color=COLOR[color] # gán màu cho color của agent
            else:
    ...
    def y(self,newY):
        ...
        if(hasattr(self,'_head')): # hàm này có sẵn trong python để check xem agent có thuộc tính head ko
            if self.footprints is False: # không in dấu chân agent ở vị trí cũ
                self._parentMaze._canvas.delete(self._head) # xóa agent
            else:
                if self.shape=='square':
                    self._parentMaze._canvas.itemconfig(self._head, fill=self.color.value[1],outline="") # tô màu dấu chân agent
                    self._parentMaze._canvas.tag_raise(self._head) # cập nhật tọa độ agent
                    try:
...
class maze:
    ...
    def CreateMaze(self,x=1,y=1,pattern=None,loopPercent=0,saveMaze=False,loadMaze=None,theme:COLOR=COLOR.dark):
        ...
        def BFS(cell):
            ...
            path = {} # lưu các ô trên đường đi ngắn nhất
            visited = {(self.rows,self.cols)} # đã thăm ô xuất phát
            while len(frontier) > 0:
                cell = frontier.popleft() # lấy ra 1 ô trong hàng đợi
        ...
        if not loadMaze:
            ...
            if(pattern is not None and pattern.lower()=='h'): # .lower để chuyển chữ hoa thành chữ thường nếu có
